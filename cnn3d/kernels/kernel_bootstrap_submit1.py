...
def kernel_video_inference_main(
    ...
    if use_fast_submit and len(videos) == 6:
        ...
        for video_fname in videos:
            ...
            if len(df.submission) and need_maximum_suppression:
                predictions_nms = keep_maximums(df.submission, iou_thresh=tracking_iou_threshold, dist=tracking_frames_distance)
            else:
...
class InferenceHelmetCropDataset(Dataset):
    def __init__(
        self,
        frames,
        bboxes,
    ...
    def _get_mini_clips(self, frames):
    ...
    def load_image(self, index):
        if index < 0 or index >= len(self.unique_ids):
...
def get_inference_model(checkpoint_path):
    model = generate_model(checkpoint_path)
    model.eval()
    return model
...
def video_classificatoin_inference_main(
    ...
    boxdf = pd.read_csv('raw_predictions.csv').query("helmet_probas >= 0.3")
    ...
    boxdf.to_csv('final_predictions.csv')
    return boxdf
...
def joint_inference(run_detection=True, run_classification=True):
    ...
    if run_classification:
        ...
        submission = submission[['gameKey', 'playID', 'view', 'video', 'frame', 'left', 'width', 'top', 'height']]
        submission.to_csv('submission.csv')
    return submission


joint_inference(False, True)
