...
from cnn3d.video_classifier_dataset import CacheDirHelmetCropDataset, CacheHelmetCropDataset, UintCacheDirHelmetCropDataset
...
def test_mini_clip_dataset(n_train_videos=1, save_clips=False):
    ...
    data_dir = "/media/anastasiya/4TB_Ext4/NFL/"
    print(data_dir)
    ...
    for video in train_videos[:1]:
        ...
        endzone = split.train_df[split.train_df.video == video]
        #endzone = split.valid_df[split.valid_df.video == video]

        print(endzone['frame'].unique())
        print(len(endzone['frame'].unique()))
        print(np.sum(endzone['impact'].fillna(0).values))
        print(len(endzone[endzone['impact'] == 1]['frame'].unique()))

        train_ds = UintCacheDirHelmetCropDataset(
            *single_video_to_dataset_args_by_box(endzone),
            ...
            impact_p=impact_p,
            mode ='r',
            cache_dir = os.path.join(data_dir, 'tensors/uint/train')
        )
        indexes_with_impact = np.flatnonzero(train_ds.labels)
        if save_clips:
            ...
            for i in range(0,50):
                ...
                #(1,2,3, 0) is for dataset
                tensor = np.transpose(sample[IMAGE_KEY].numpy(), (1, 2, 3, 0))#(2, 1, 0, 3)) -dont know when.
