...
def make_folds(
    df: pd.DataFrame, n_splits: int = 5, save_dir: Optional[str] = None
) -> pd.DataFrame:
    """
    Create folds
    Args: 
    ...
    df_folds.loc[:, "video"] = (
        df[["image_name", "video"]].groupby("image_name").min()["video"]
    )
...
def fix_errors(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.loc[
        (df.video == "58098_001193_Endzone.mp4")
        & (df.frame == 40)
        & (df.label == "H33"),
        "impact",
    ] = 0
    ...
    df.loc[
        (df.video == "57911_000147_Endzone.mp4")
        & (df.frame == 113)
        & (df.label == "H21"),
        "impact",
    ] = 0
    ...
    df.loc[
        df.video == "57584_000336_Sideline.mp4", "frame"
    ] += 1  # This video for some reason starts with 0
...
def filter_impacts(df: pd.DataFrame) -> pd.DataFrame:
    mask = ((df["visibility"] != 0) & (df["confidence"] != 1) & (df["impact"] > 0)) | (
        df["impact"] == 0
    )
...
def preprocess_video_meta(
    video_labels: pd.DataFrame, save_dir: Optional[str] = None
) -> pd.DataFrame:
    ...
    for row in tqdm(video_labels_with_impact[["video", "frame", "label"]].values):
        ...
        video_labels.loc[
            (video_labels["video"] == row[0])
            & (video_labels["frame"].isin(frames))
            & (video_labels["label"] == row[2]),
            "impact",
        ] = 1

    video_labels["image_name"] = (
        video_labels["video"].str.replace(".mp4", "")
        + "_"
        + video_labels["frame"].astype(str)
        + ".png"
    )
