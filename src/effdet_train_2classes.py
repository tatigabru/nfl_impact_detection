...
class DatasetRetriever(Dataset):
    ...
    def load_image_and_boxes(self, index):
        image_id = self.image_ids[index]
        image = (
            cv2.imread(f"{TRAIN_VIDEO}/{image_id}", cv2.IMREAD_COLOR)
            .copy()
            .astype(np.float32)
        )
...
def run_training() -> None:
    ...
    neptune.append_tags(
        ["images", "no padding", "video_meta_2", "filtered_impacts", "train_augs"]
    )
    device = (
    ...
    weights_path = (
        "../../checkpoints/effdet5_fold_0_512_run3/best-checkpoint-027epoch.bin"
    )
