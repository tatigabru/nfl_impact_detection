...
def make_test_frames():
    ...
    pool = Pool()
    pool.map(
        write_frames, map(lambda video_name: f"{video_dir}/{video_name}", test_videos)
    )
...
def make_video_from_frames(video_name, imade_dir, start, stop):
    ...
    writer = cv2.VideoWriter(
        video_name, cv2.VideoWriter_fourcc(*VIDEO_CODEC), 30, (1280, 720)
    )
...
def create_meta_nfl_helmets_hits():
    ...
    """
    df = pd.DataFrame(
        columns=["video", "frame", "impact_type", "left", "top", "right", "bottom"]
    )
    rows = []
    rows.append(
        ["hit279.mp4", 279 - 267 + 1, "head", 754, 325, 768, 335]
    )  # only one visible
    ...
    rows.append(
        ["hit3194.mp4", 3194 - 3032 + 1, "head", 625, 250, 638, 276]
    )  # not very visible, behind the first one
