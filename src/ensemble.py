...
FOLD0 = [
    ...
    "57584_002674_Sideline.mp4",
    "57586_001934_Endzone.mp4",
    "57586_001934_Sideline.mp4",
    "57594_000923_Endzone.mp4",
    "57594_000923_Sideline.mp4",
    "57680_002206_Endzone.mp4",
    "57680_002206_Sideline.mp4",
    "57686_002546_Endzone.mp4",
    "57686_002546_Sideline.mp4",
    "57784_001741_Endzone.mp4",
    "57784_001741_Sideline.mp4",
    "57787_003413_Endzone.mp4",
    "57787_003413_Sideline.mp4",
    ...
    "57913_000218_Sideline.mp4",
    "57992_000350_Endzone.mp4",
...
MIX_FOLD0 = [
    "../../preds/final_predictions_resnet.csv",
    "../../preds/densenet121_val_impactp01_fold0.csv",
]

MIX = [
    "../../preds/3dnn_predictions_fold0.csv",
    # '../../preds/densenet121_val_impactp01_fold0.csv',
    # '../../preds/b5_val_impactp01_fold0.csv'
]

MIX_FOLD2 = [
    "../../preds/densenet121_val_impactp01_fold2.csv",
...
PRED_HITS = [
    f"../../preds/densenet121_hits_impactp01_fold{fold}.csv" for fold in range(4)
]
...
def show_boxes(
    image, image_id, boxes_list, scores_list, labels_list, image_size=(720, 1280)
):
    thickness = 1
    colors_to_use = [
        (255, 0, 0),
...
def load_image_preds(image_id: str, preds: pd.DataFrame, image_shape=(720, 1280)):
    ...
    coords2norm = np.array(
        [[1.0 / image_width, 1.0 / image_height, 1.0 / image_width, 1.0 / image_height]]
    )
...
def ensemble_boxes_per_frame(
    dfs: List[pd.DataFrame], iou_threshold=0.7
) -> pd.DataFrame:
    """
    ...
    for video in videos:
        ...
        for frame in frames:
            ...
            for df in video_dfs:
                ...
                boxes = np.stack(
                    [
                        frame_df["left"].values,
                        frame_df["top"].values,
                        frame_df["right"].values,
                        frame_df["bottom"].values,
                    ],
                    axis=1,
                )
            ...
            if len(bboxes_list) > 1:
                boxes, labels, scores = merge_bboxes(
                    bboxes_list,
                    labels_list,
                    scores_list,
...
def plot_wbf_image_preds(
    image_path: str, image_id: str, dfs, weights, iou_thr, skip_box_thr
):
    """Ensemble boxes for a single image"""
    boxes_list, scores_list, labels_list = [], [], []
    # combine all preds for an image
    ...
    show_boxes(
        image.copy(), image_id[:-4] + "_raw", boxes_list, scores_list, labels_list
    )
...
def preprocess_df(df: pd.DataFrame) -> pd.DataFrame:
    df["label"] = 1
    df["image_name"] = (
        df["video"].str.replace(".mp4", "") + "_" + df["frame"].astype(str) + ".png"
    )
...
def combine_preds(
    images: list, df: pd.DataFrame, dfs, image_size, weights, iou_thr, method="softnms"
) -> pd.DataFrame:
    """
    ...
    for image_id in images:
        ...
        boxes, scores, labels = merge_image_preds(
            image_id, dfs, weights, image_size, iou_thr, method
        )
...
def combine_preds_wbf(
    images: list, df: pd.DataFrame, dfs, image_size, weights, iou_thr, skip_box_thr
) -> pd.DataFrame:
    """
    Helper, to combine raw predicitons for all images
    Args:
    ...
    for image_id in images:
        ...
        gameKey, playID, view, frame = image_id.split("_")[:4]
        video = f"{gameKey}_{playID}_{view}.mp4"
        boxes, scores, labels = wbf_image_preds(
            image_id, dfs, weights, iou_thr, skip_box_thr
        )
        for (x1, y1, x2, y2), score, label in zip(boxes, scores, labels):
            df.loc[row, "gameKey"] = gameKey
            df.loc[row, "playID"] = int(playID)
...
def test_wbf(image_id, dfs, weights, iou_thr, skip_box_thr):
    ...
    boxes, scores, labels = plot_wbf_image_preds(
        image_path, image_id, dfs, weights, iou_thr, skip_box_thr
    )
...
def grid_search_wbf(
    ...
    for skip_box_thr in skip_box_wbf_params:
        for iou_thr in iou_wbf_params:
            ...
            df_combo = combine_preds_wbf(
                images, df_combo, dfs, image_size, weights, iou_thr, skip_box_thr
            )
            print("Apply filtering after...")
            df_combo = df_combo[df_combo.scores > IMPACT_THRESHOLD_SCORE]
            print("Apply postprocessing...")
            df_keepmax = keep_maximums(
                df_combo,
                iou_thresh=TRACKING_IOU_THRESHOLD,
            ...
            print(
                f"EXPERIMENT {num}, thres {IMPACT_THRESHOLD_SCORE} \n Precision {prec}, recall {rec}, f1 {f1}"
            )
            num += 1
            if f1 > best_metric:
                best_metric = f1
                ...
                print(
                    f"Current best params: wbf_iou {best_iou}, skip threshold {best_skip}"
                )
...
def grid_search_tracking(
    dfs, gtdf: pd.DataFrame, images: list, weights: list, save_dir="../../ensembling"
):
    """
    ...
    for dist in dist_params:
        for track_iou_thr in track_iou_params:
            ...
            prec, rec, f1 = evaluate_df(gtdf, df_keepmax, video_names=None, impact=True)
            print(
                f"EXPERIMENT {num}, iou track thres {track_iou_thr}, dist {dist}, thres {IMPACT_THRESHOLD_SCORE} \n Precision {prec}, recall {rec}, f1 {f1}"
            )
            num += 1
            if f1 > best_metric:
                best_metric = f1
...
def do_val_preds(dfs, gtdf: pd.DataFrame):
    ...
    df_keepmax = keep_maximums(
        df_combo, iou_thresh=TRACKING_IOU_THRESHOLD, dist=TRACKING_FRAMES_DISTANCE
    )
    # df_keepmax.to_csv('../../preds/keepmax_wbf_densenet121.csv', index = False)
    video_names = gtdf["video"].unique()
    pred_video = df_keepmax["video"].unique()
    print(
        "Number of videos for evaluation:",
        len(video_names),
        " predicted video",
        len(pred_video),
    )
...
def grid_impact_threshold(dfs, gtdf: pd.DataFrame):
    ...
    for impact_thres in impact_thres_params:
        ...
        prec, rec, f1 = evaluate_df(gtdf, df_keepmax, video_names=None, impact=True)
        print(
            f"'EXPERIMENT {num}, thres {impact_thres} \n Precision {prec}, recall {rec}, f1 {f1}"
        )
        num += 1
...
if __name__ == "__main__":
    ...
    gtdf = gtdf[
        (gtdf["impact"] == 1) & (gtdf["confidence"] > 1) & (gtdf["visibility"] > 0)
    ]  # only confident impacts
