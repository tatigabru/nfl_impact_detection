...
def calculate_final_score(
    ...
    iou_thresholds: list = [0.5, 0.55, 0.6, 0.65, 0.7, 0.75],
) -> float:
    final_scores = []
...
def plot_result(sample, preds, gt_boxes):
    ...
    for gt_box in gt_boxes:
        cv2.rectangle(
            sample, (gt_box[0], gt_box[1]), (gt_box[2], gt_box[3]), (0, 0, 220), 2
        )
    ax.set_axis_off()
    ax.imshow(sample)
...
def map_iou(
    boxes_true, boxes_pred, scores, thresholds=[0.5, 0.55, 0.6, 0.65, 0.7, 0.75]
):
    """
    ...
    assert (
        boxes_true.shape[1] == 4 or boxes_pred.shape[1] == 4
    ), "boxes should be 2D arrays with shape[1]=4"
    if len(boxes_pred):
        assert len(scores) == len(
            boxes_pred
        ), "boxes_pred and scores should be same length"
...
def competition_metric(
    true_boxes: np.array, pred_boxes: np.array, pred_scores: np.array, score_thr: float
) -> float:
    """
    Mean average precision at differnet intersection over union (IoU) threshold
    ...
    for ind in range(n_images):
        ...
        # true positive
            ...
            contrib = map_iou(
                cur_image_true_boxes, cur_image_pred_boxes, cur_pred_scores
            )
...
def find_best_nms_threshold(
    true_list,
    pred_boxes,
    pred_scores,
    min_thres: float = 0.30,
    max_thres: float = 0.40,
    points: int = 10,
) -> Tuple[float, float]:
