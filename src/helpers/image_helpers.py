...
def normalize_4_channels(
    img: np.array,
    mean: list = [0.485, 0.456, 0.406, 0.406],
    std: list = [0.229, 0.224, 0.225, 0.225],
...
def plot_mixup_detection(subplots: Tuple[int] = (3, 3),) -> plt.figure:
    ...
    for i in range(subplot_h):
        image, boxes = dataset.load_image_and_boxes(
            random.randint(0, dataset.image_ids.shape[0] - 1)
        )
        r_image, r_boxes = dataset.load_image_and_boxes(
            random.randint(0, dataset.image_ids.shape[0] - 1)
        )
