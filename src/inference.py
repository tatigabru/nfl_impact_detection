...
from effdet import DetBenchEval, DetBenchTrain, EfficientDet, get_efficientdet_config
...
def run_inference() -> None:
    ...
    # save predictions
    box_df = pd.DataFrame(
        np.concatenate(results_boxes), columns=["left", "top", "width", "height"]
    )
    test_df = pd.DataFrame(
        {"scores": np.concatenate(results_scores), "image_name": result_image_ids}
    )
    ...
    test_df["gameKey"] = (
        test_df.image_name.astype(str).str.split("_").str[0].astype(int)
    )
